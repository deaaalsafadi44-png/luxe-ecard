...
/**
 * Wedding gallery: upload images from device (UploadThing); previews and remove per image.
 */
export function GalleryPhotoUploadField({
  ...
  onUploadError: () => void;
  /** When invitation exists, save gallery URLs after upload or remove. */
  onPersist?: (next: string[]) => Promise<void>;
}) {
  const { t } = useI18n();
  const remaining = MAX_GALLERY - urls.length;
          ...
          {urls.map((url, index) => (
            ...
            >
              <div className="relative aspect-square w-full">
                {/* eslint-disable-next-line @next/next/no-img-element -- UploadThing CDN URLs */}
                <img
                  src={url}
                  alt=""
        ...
        <div className="rounded-xl border border-royal-gold/25 bg-white p-3 sm:p-4">
          <UploadThingButton
            ...
            onClientUploadComplete={(files) => {
              ...
              const previous = urls;
              const merged = [...urls, ...added].slice(0, MAX_GALLERY);
              onUrlsChange(merged);
              if (onPersist) {
