...
import { saveCoupleAuth } from "@/lib/authStorage";
  ...
  return (
    <main className="flex min-h-dvh flex-col justify-center bg-royal-cream px-4 py-10 text-royal-brown sm:px-6 md:px-8">
      <section className="mx-auto w-full max-w-md rounded-2xl border border-royal-gold/30 bg-white p-6 shadow-sm sm:p-8">
        <h1 className="text-balance text-xl font-semibold sm:text-2xl">{t("coupleLoginTitle")}</h1>
        <p className="mt-2 text-pretty text-sm text-royal-brown/80">{t("coupleEditIntro")}</p>
        ...
        <div className="mt-6 flex flex-col gap-2 text-sm">
          <Link href="/" className="text-royal-brown/70 hover:underline">
            {t("backToHome")}
          </Link>
        </div>
      </section>
